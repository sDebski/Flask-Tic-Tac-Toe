...
def handle_user_join(player_id, first_name, session_id):
    ...
    if len(Rooms) == 0:
        ...
        for room in Rooms:
            if room.get_players() == 1 and room.player1_id != player_id:
                join_room(room.id)
                ...
                data = {
                    "player1_first_name": room.player1_first_name,
                ...
                emit(
                    "player_turn",
                    {"my_turn": room.player1_id, "opponent": room.player2_id},
                    to=room.id,
                )
                break
        else:
...
def handle_user_move(field_id, sign, user_id, room_id):
    ...
    if not result:
        if user_id == room.player1_id:
            emit(
                "player_turn",
                {"my_turn": room.player2_id, "opponent": room.player1_id},
                to=room_id,
            )
        else:
            emit(
...
def disconnect_details(user_id):
    print("User disconnected")
    for room in Rooms:
        if room.player1_id == user_id or room.player2_id == user_id:
            if room.get_players() == 2:
                winner_id = (
                    room.player1_id if user_id == room.player2_id else room.player2_id
                )
                data = {
                    "winner_id": winner_id,
                    "p1": room.player1_id,
...
def checkIfGameFinished(room, user_id):
    ...
    if (
        ...
        emit("game_finished", data, to=room.id)
        disconnect_details(user_id=user_id)
        return True
